			...
			if(child!=NULL){
				...
				switch(method){
				    ...
				    case GeneralizedAStarSearch:
                    	// Compute heuristic for the child node
                    	child->state.h_n = Compute_Heuristic_Function(&(child->state), goal_state);
                    	if(temp_node!=NULL){
						    // f(n) = (1-alpha)*g(n) + alpha*h(n)
						    if( (1.0f-alpha)*child->path_cost + alpha*child->state.h_n < 
						        (1.0f-alpha)*temp_node->path_cost + alpha*temp_node->state.h_n )
							    Remove_Node_From_Frontier(temp_node, &frontier);
						    else
						        continue;
					    }
...
//______________________________________________________________________________
// Generalized A*: f(n) = (1-alpha)*g(n) + alpha*h(n)
// alpha=0.0 --> Uniform Cost Search (only g)
// alpha=0.5 --> Standard A*
// alpha=1.0 --> Greedy Search (only h)
...
{  
	...
	// Evaluation function for this child: f(n) = (1-alpha)*g(n) + alpha*h(n)
	float f_child = (1.0f-alpha)*child->path_cost + alpha*child->state.h_n;
    ...
    if(Empty(*frontier)){
		...
		float f_front = (1.0f-alpha)*(*frontier)->node->path_cost + alpha*(*frontier)->node->state.h_n;
	    if(f_child < f_front){ // Child has the lowest evaluation function value
	        new_queue->next = *frontier;
            *frontier = new_queue; 
        }
        else{
            for(temp_queue = *frontier; temp_queue->next != NULL; temp_queue = temp_queue->next){
            	float f_next = (1.0f-alpha)*temp_queue->next->node->path_cost + alpha*temp_queue->next->node->state.h_n;
                if(f_child < f_next){ 
                     new_queue->next = temp_queue->next;   
                     temp_queue->next = new_queue;
                     return;
